...
def datafr():
    df= pd.read_csv("C:/Users/SunLand/Desktop/New folder/Airbnb/Airbnb NYC 2019.csv")
    return df
...
if select == "Home":
    ...
    image1= Image.open("C:/Users/SunLand/Desktop/images/images1.jpeg")
    st.image(image1)
...
if select == "Data Exploration":
    ...
    with tab1:
        ...
        with col1:
            ...
            neighbourhood= st.selectbox("Select the neighbourhood",df["neighbourhood"].unique())


            df1= df[df["neighbourhood"] == neighbourhood]
            ...
            df_bar= pd.DataFrame(df2.groupby("property_type")[["price","reviews_per_month","number_of_reviews"]].sum())
            df_bar.reset_index(inplace= True)

            fig_bar= px.bar(df_bar, x='property_type', y= "price", title= "PRICE FOR PROPERTY_TYPES",hover_data=["number_of_reviews","reviews_per_month"],color_discrete_sequence=px.colors.sequential.Redor_r, width=600, height=500)
    ...
    with tab2:
        ...
        def datafr():
            df_a= pd.read_csv("C:/Users/SunLand/Desktop/New folder/Airbnb/Airbnb NYC 2019.csv")
            return df_a
        ...
        with col1:
            ...
            neighbourhood_a= st.selectbox("Select the neighbourhood_a",df_a["neighbourhood"].unique())

            df1_a= df[df["neighbourhood"] == neighbourhood_a]
    ...
    with tab3:
        ...
        neighbourhood_b= st.selectbox("Select the neighbourhood_b",df_l["neighbourhood"].unique())

        df1_l= df_l[df_l["neighbourhood"] == neighbourhood_b]
    ...
    with tab4:
        ...
        fig_4 = px.scatter_mapbox(df, lat='latitude', lon='longitude', color='price',
    ...
    with tab5:
        ...
        neighbourhood_d= st.selectbox("Select the neighbourhood_d",df["neighbourhood"].unique())

        df1_t= df[df["neighbourhood"] == neighbourhood_d]
        ...
        df_price= pd.DataFrame(df2_t_sorted.groupby("is_location_exact")["price"].agg(["sum","mean"]))
        df_price.reset_index(inplace= True)
        df_price.columns= ["is_location_exact", "Total_price", "Avarage_price"]
        
        col1, col2= st.columns(2)

        with col1:
            
            fig_price= px.bar(df_price, x= "Total_price", y= "is_location_exact", orientation='h',
                            title= "PRICE BASED ON is_location_exact", width= 600, height= 800)
            st.plotly_chart(fig_price)

        with col2:

            fig_price_2= px.bar(df_price, x= "Avarage_price", y= "is_location_exact", orientation='h',
        ...
        with col1:
            ...
            df_price_1= pd.DataFrame(df2_t_sorted.groupby("is_location_exact")["price"].agg(["sum","mean"]))
            df_price_1.reset_index(inplace= True)
            df_price_1.columns= ["is_location_exact", "Total_price", "Avarage_price"]
            
            fig_price_3= px.bar(df_price_1, x= "Total_price", y= "is_location_exact", orientation='h',
                                width= 600,height= 800,color_discrete_sequence=px.colors.sequential.Bluered_r,
                                title= "PRICE BASED ON is_location_exact")
            st.plotly_chart(fig_price_3)

        with col2:

            fig_price_4= px.bar(df_price_1, x= "Avarage_price", y= "is_location_exact", orientation='h',
                                width= 600, height= 800,color_discrete_sequence=px.colors.sequential.Bluered_r,
                                title= "AVERAGE PRICE BASED ON is_location_exact")
